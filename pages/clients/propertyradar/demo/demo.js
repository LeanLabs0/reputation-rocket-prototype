 ...
 *   2. Guided coach-mark tour tailored to the PropertyRadar walkthrough.
 *
...
(function () {
  ...
  const SESSION_KEY = 'rr_session_pr-demo';
  ...
  window.fetch = function (input, init) {
    ...
    if (url.indexOf('/api/notify') !== -1) {
      return jsonResponse({ ok: true, delivered_to: 'pr-demo' }, 200);
    }
    if (url.indexOf('/api/upload-video') !== -1) {
      return jsonResponse({ ok: true, id: 'pr-demo-video', url: '#' }, 500);
    }
  ...
  const INTRO = {
    ...
    body: 'You’re about to walk through exactly what your happy customers experience with <strong>PropertyRadar</strong>. Review sites in this demo are dummy placeholders — nothing is posted for real. We’ll pop in with a quick tip at each step.',
  };
