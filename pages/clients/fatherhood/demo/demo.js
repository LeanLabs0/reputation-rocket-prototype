 ...
 *   2. Guided coach-mark tour tailored to the National Fatherhood Initiative walkthrough.
 *
...
(function () {
  ...
  const SESSION_KEY = 'rr_session_nfi-demo';
  ...
  window.fetch = function (input, init) {
    ...
    if (url.indexOf('/api/notify') !== -1) {
      return jsonResponse({ ok: true, delivered_to: 'nfi-demo' }, 200);
    }
    if (url.indexOf('/api/upload-video') !== -1) {
      return jsonResponse({ ok: true, id: 'nfi-demo-video', url: '#' }, 500);
    }

    return originalFetch(input, init);
  };

  // Video step is disabled for NFI — tour still uses the same copy as /lean-labs/demo/.
  const STEPS = {
    welcome: {
      target: '#btn-start',
      count: 'Step 1 of 6',
  ...
  const INTRO = {
    ...
    body: 'You’re about to walk through exactly what your happy supporters experience with <strong>National Fatherhood Initiative</strong>. Review sites in this demo are dummy placeholders — nothing is posted for real. We’ll pop in with a quick tip at each step.',
  };
