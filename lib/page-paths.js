...
function resolvePublicFile(pathname) {
  ...
  if (clean === '/404.html' || clean === '/404' || clean === '/404/') {
...
function buildClientRewrites(slugs = listClientSlugs()) {
  ...
  for (const slug of slugs) {
    rewrites.push(
      // Nested demo/ must hit index.html. A rewrite to the folder 404s on Vercel.
      { source: `/${slug}/demo`, destination: `/pages/clients/${slug}/demo/index.html` },
      { source: `/${slug}/demo/`, destination: `/pages/clients/${slug}/demo/index.html` },
...
function buildNetlifyRedirectLines(slugs = listClientSlugs()) {
  ...
  for (const slug of slugs) {
    lines.push(`/${slug}/demo  /pages/clients/${slug}/demo/index.html  200`);
    lines.push(`/${slug}/demo/  /pages/clients/${slug}/demo/index.html  200`);
