...
const PLATFORM_ID_RE = /^[a-z][a-z0-9-]{0,62}$/;
...
function normalizePlatforms(value) {
  ...
  for (const raw of list) {
    ...
    // Keep catalog ids, and also slug-shaped ids so a stale local-dev process
    // cannot silently drop a platform the configure UI already shows.
    if (!PLATFORM_IDS.has(id) && !PLATFORM_ID_RE.test(id)) continue;
    out.push(id);
  }
  return out;
}
