...
function writePortalSettingsToConfigFile(clientSlug, settings, extras = {}) {
  ...
  if (existing.slackChannel) next.slackChannel = existing.slackChannel;
  if (existing.slackThreadPositive) next.slackThreadPositive = existing.slackThreadPositive;
  if (existing.slackThreadNegative) next.slackThreadNegative = existing.slackThreadNegative;
  if (Array.isArray(existing.notifyEmails) && existing.notifyEmails.length) {
    next.notifyEmails = existing.notifyEmails;
  }
