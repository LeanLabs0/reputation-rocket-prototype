 ...
 */
function patchClientHubSpotConfig(clientSlug, {
  portalId,
  formId,
  formRegion,
  hubspotCompleteProperty,
  hubspotCompleteValue,
  hubspotIncompleteValue,
  hubspotOutcomeProperty,
  hubspotOutcomePositiveValue,
  hubspotOutcomeNegativeValue,
} = {}) {
  ...
  try {
    ...
    const set = (key, value) => {
      ...
      if (re.test(src)) {
        ...
        src = src.replace(
          /hubspotFormRegion\s*:\s*(['"`])[^'"`]*\1/,
          (m) => `${m},\n  ${key}: '${String(value).replace(/'/g, "\\'")}'`,
        );
      }
    };
    set('hubspotPortalId', portalId);
    set('hubspotFormId', formId);
    set('hubspotFormRegion', formRegion);
    set('hubspotCompleteProperty', hubspotCompleteProperty);
    set('hubspotCompleteValue', hubspotCompleteValue);
    set('hubspotIncompleteValue', hubspotIncompleteValue);
    set('hubspotOutcomeProperty', hubspotOutcomeProperty);
    set('hubspotOutcomePositiveValue', hubspotOutcomePositiveValue);
    set('hubspotOutcomeNegativeValue', hubspotOutcomeNegativeValue);
