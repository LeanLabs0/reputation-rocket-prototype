...
async function readRawStore() {
  if (hasKvStore()) {
    const res = await kvCommand('GET', [STORE_KEY]);
  ...
  // On Vercel the filesystem is read-only — never mkdir here.
  if (process.env.VERCEL) return { clients: {} };
...
async function writeRawStore(data) {
  ...
  if (hasKvStore()) {
    await kvCommand('SET', [STORE_KEY, payload]);
    return;
  }
  if (process.env.VERCEL) {
    throw new Error(
      'Cannot write configure store on Vercel without KV. Set KV_REST_API_URL and KV_REST_API_TOKEN.',
    );
  }
  ensureLocalDir();
...
function kvRestUrl() {
  return String(process.env.KV_REST_API_URL || '').trim();
}
...
function assertWritableStore() {
  if (process.env.VERCEL && !hasKvStore()) {
    throw new Error(
      'Configure storage is not persistent on Vercel without KV. Set KV_REST_API_URL and KV_REST_API_TOKEN.',
    );
  }
}
